...
The conditions are:
   ...
   * $$\scriptsize \tau_{\text{safe_col}} =  \text{recvOneCmd}   \wedge  ( \text{recvCartImpCmd}  \vee   \text{recvJntImpCmd} )  \wedge  \neg   \text{inSelfCollision} $$
   * $$\scriptsize \epsilon_{\text{safe_col}} =  \neg   \text{CURRENT_BEHAVIOR_OK}  \vee   \neg   \text{veBodyStatusValid}  \vee   \text{veBodyInSafeState}$$
   ...
   * $$\scriptsize \tau_{\text{cart_imp}} = ( \text{recvOneCmd}   \wedge  ( \text{recvJntImpCmd}  \vee   \text{recvSafeColCmd} ))  \vee   \text{inSelfCollision} $$
   * $$\scriptsize \epsilon_{\text{cart_imp}} =  \neg   \text{CURRENT_BEHAVIOR_OK} \vee   \neg   \text{veBodyStatusValid}   \vee  ( \text{veBodyInSafeState} )$$
   ...
   * $$\scriptsize \tau_{\text{jnt_imp}} = ( \text{recvOneCmd}   \wedge  ( \text{recvCartImpCmd}  \vee   \text{recvSafeColCmd} ))  \vee   \text{inSelfCollision} $$
   * $$\scriptsize \epsilon_{\text{jnt_imp}} =   \neg   \text{CURRENT_BEHAVIOR_OK} \vee   \neg   \text{veBodyStatusValid}  \vee  ( \text{veBodyInSafeState} )$$
